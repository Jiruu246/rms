...
func (_q *MediaAssetQuery) QueryUploadedBy() *UserQuery {
...
func (_q *MediaAssetQuery) QueryUpload() *MediaUploadQuery {
...
func (_q *MediaAssetQuery) WithUploadedBy(opts ...func(*UserQuery)) *MediaAssetQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *MediaAssetQuery) WithUpload(opts ...func(*MediaUploadQuery)) *MediaAssetQuery {
	query := (&MediaUploadClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(mediaasset.FieldUpdateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.MediaAsset.Query().
//		Select(mediaasset.FieldUpdateTime).
//		Scan(ctx, &v)
...
func (_q *MediaAssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MediaAsset, error) {
	...
	if query := _q.withUploadedBy; query != nil {
		if err := _q.loadUploadedBy(ctx, query, nodes, nil,
			func(n *MediaAsset, e *User) { n.Edges.UploadedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUpload; query != nil {
		if err := _q.loadUpload(ctx, query, nodes, nil,
			func(n *MediaAsset, e *MediaUpload) { n.Edges.Upload = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaAssetQuery) loadUploadedBy(ctx context.Context, query *UserQuery, nodes []*MediaAsset, init func(*MediaAsset), assign func(*MediaAsset, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*MediaAsset)
	for i := range nodes {
		fk := nodes[i].UploadedByUserID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "uploaded_by_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *MediaAssetQuery) loadUpload(ctx context.Context, query *MediaUploadQuery, nodes []*MediaAsset, init func(*MediaAsset), assign func(*MediaAsset, *MediaUpload)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*MediaAsset)
	for i := range nodes {
		fk := nodes[i].UploadID
	...
	query.Where(mediaupload.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *MediaAssetQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUploadedBy != nil {
			_spec.Node.AddColumnOnce(mediaasset.FieldUploadedByUserID)
		}
		if _q.withUpload != nil {
			_spec.Node.AddColumnOnce(mediaasset.FieldUploadID)
		}
	}
