...
- Update endpoints use `PATCH`, not `PUT` — updates always modify a subset of a resource's attributes rather than replacing it wholesale, so `@Router` blocks and `internal/server/server.go` route registrations must match (`[patch]` / `.PATCH(...)`).
