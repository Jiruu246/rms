...
func (m *MockRefreshTokenRepository) GetByToken(ctx context.Context, token string) (*ent.RefreshToken, error) {
...
func TestAuthService_Login(t *testing.T) {
    ...
    for _, testCase := range testCases {
        t.Run(testCase.name, func(t *testing.T) {
            ...
            service := NewAuthService(mockUserRepo, mockRefreshRepo)
            result, err := service.Login(context.Background(), testCase.req, []byte("secret"))
...
func TestAuthService_RefreshAccessToken(t *testing.T) {
    ...
    mockRefreshRepo.On("GetByToken", mock.Anything, "refresh-token").Return(&ent.RefreshToken{
        ID:     tokenID,
        UserID: userID,
    }, nil)
    ...
    service := NewAuthService(mockUserRepo, mockRefreshRepo)
    response, err := service.RefreshAccessToken(context.Background(), "refresh-token", []byte("secret"))
...
func TestAuthService_Logout(t *testing.T) {
    ...
    mockRefreshRepo.On("GetByToken", mock.Anything, "refresh-token").Return(&ent.RefreshToken{ID: tokenID}, nil)
