...
func (m *MockUserRepository) Create(ctx context.Context, user *ent.Customer) (*ent.Customer, error) {
	args := m.Called(ctx, user)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*ent.Customer), args.Error(1)
}

func (m *MockUserRepository) GetByEmail(ctx context.Context, email string) (*ent.Customer, error) {
...
func (m *MockUserRepository) GetByID(ctx context.Context, id uuid.UUID) (*ent.Customer, error) {
...
func (m *MockUserRepository) Update(ctx context.Context, user *ent.Customer) (*ent.Customer, error) {
	args := m.Called(ctx, user)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			mockSetup: func(mockRepo *MockUserRepository) {
				mockRepo.On("Create", mock.Anything, mock.Anything).Return(&ent.Customer{
					Name:  "John Doe",
					Email: "john.doe@example.com",
				}, nil)
			},
...
func TestUserService_Login(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			mockSetup: func(mockRepo *MockUserRepository) {
				mockRepo.On("GetByEmail", mock.Anything, "john.doe@example.com").Return(&ent.Customer{
					Email:        "john.doe@example.com",
...
func TestUserService_GetProfile(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			idInput: testUserId,
			mockSetup: func(mockRepo *MockUserRepository) {
				mockRepo.On("GetByID", mock.Anything, testUserId).Return(&ent.Customer{
					ID: testUserId,
				}, nil)
			},
			expectedError: "",
	...
	testCases := []struct {
	...
	}{
		{
			...
			updateInput: &dto.UpdateUserRequest{
				...
				Email: ptr("updated.email@example.com"),
			},
			mockSetup: func(mockRepo *MockUserRepository) {
				mockRepo.On("GetByID", mock.Anything, mock.Anything).Return(&ent.Customer{
					ID: uuid.New(),
				}, nil)
				mockRepo.On("Update", mock.Anything, mock.Anything).Return(&ent.Customer{
					Name:  "Updated Name",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.expectedError != "" {
				...
				assert.Equal(t, *testCase.updateInput.Email, result.Email)
			}
