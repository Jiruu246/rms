...
func (_q *OrderItemQuery) QueryMenuItem() *MenuItemQuery {
...
func (_q *OrderItemQuery) QueryOrderItemModifierOptions() *OrderItemModifierOptionQuery {
...
func (_q *OrderItemQuery) WithMenuItem(opts ...func(*MenuItemQuery)) *OrderItemQuery {
	query := (&MenuItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderItemQuery) WithOrderItemModifierOptions(opts ...func(*OrderItemModifierOptionQuery)) *OrderItemQuery {
	query := (&OrderItemModifierOptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if query := _q.withMenuItem; query != nil {
		if err := _q.loadMenuItem(ctx, query, nodes, nil,
			func(n *OrderItem, e *MenuItem) { n.Edges.MenuItem = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOrderItemModifierOptions; query != nil {
		if err := _q.loadOrderItemModifierOptions(ctx, query, nodes,
			func(n *OrderItem) { n.Edges.OrderItemModifierOptions = []*OrderItemModifierOption{} },
			func(n *OrderItem, e *OrderItemModifierOption) {
...
func (_q *OrderItemQuery) loadMenuItem(ctx context.Context, query *MenuItemQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *MenuItem)) error {
...
func (_q *OrderItemQuery) loadOrderItemModifierOptions(ctx context.Context, query *OrderItemModifierOptionQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *OrderItemModifierOption)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderitemmodifieroption.FieldOrderItemID)
	}
	query.Where(predicate.OrderItemModifierOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(orderitem.OrderItemModifierOptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_item_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *OrderItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withMenuItem != nil {
			_spec.Node.AddColumnOnce(orderitem.FieldMenuItemID)
		}
	}
