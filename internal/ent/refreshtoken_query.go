...
type RefreshTokenQuery struct {
	...
	withUser            *UserQuery
	withReplacedByToken *RefreshTokenQuery
...
func (_q *RefreshTokenQuery) QueryReplacedByToken() *RefreshTokenQuery {
...
func (_q *RefreshTokenQuery) WithReplacedByToken(opts ...func(*RefreshTokenQuery)) *RefreshTokenQuery {
	query := (&RefreshTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(refreshtoken.FieldUpdateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.RefreshToken.Query().
//		Select(refreshtoken.FieldUpdateTime).
//		Scan(ctx, &v)
...
func (_q *RefreshTokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RefreshToken, error) {
	...
	if query := _q.withReplacedByToken; query != nil {
		if err := _q.loadReplacedByToken(ctx, query, nodes, nil,
			func(n *RefreshToken, e *RefreshToken) { n.Edges.ReplacedByToken = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RefreshTokenQuery) loadReplacedByToken(ctx context.Context, query *RefreshTokenQuery, nodes []*RefreshToken, init func(*RefreshToken), assign func(*RefreshToken, *RefreshToken)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*RefreshToken)
	for i := range nodes {
		if nodes[i].ReplacedBy == nil {
			continue
		}
		fk := *nodes[i].ReplacedBy
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "replaced_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RefreshTokenQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withReplacedByToken != nil {
			_spec.Node.AddColumnOnce(refreshtoken.FieldReplacedBy)
		}
	}
